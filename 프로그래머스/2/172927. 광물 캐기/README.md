...
곡괭이로 5개가 되는 순간 다음 곡괭이를 선택하는 방식으로 약간의 하드코딩을 통해 문제를 접근했다. 다이아몬드 - 철 - 돌 순으로 남아있으면 순서대로 곡괭이를 선택하게 했는데 이 부분이 잘못되었다.

입력 값 범위가 작아서 완전탐색하는 방향으로 선택해도 되었고, 주어진 곡괭이와 광물의 피로도 관계를 미리 정의를 해서 피로도 누적합을 비교해 가장 작은 수를 출력하는 원리로 문제를 해결하면 됐었다.. 막상 풀이를 보니 간단한데 너무 오래 걸렸다, 구현 아이디어에서 적절한 방법을 선택하는 것이 중요한 것 같다 ㅠ
