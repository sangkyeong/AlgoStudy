...
문제 이해하는데 먼저 30분이상 걸렸다.. 
1. 정해진 범위만큼 시계방향 90도 회전 → 
2. ‘이후 얼음이 있는 칸 3개 또는 그 이상과 인접해있지 않은 칸은 얼음의 양이 1 줄어든다.’

이걸 해석하기 위해 문장을 그룹화했다

‘얼음이 있는 칸 3개 이상과 인접해 있지 않은 칸은 얼음의 양이 1줄어든다.’

⇒결국 인접한 얼음이 3개 미만이면 녹는다는 것(이게 헷갈렸다..ㅎㅋㅋ) 

3. 얼음덩어리는 bfs로 구하는 쪽으로 접근했다.


하지만 또 만난 난관은 90도 회전이었다.
전체를 90도 돌리는게 아닌 정해진 크기마다 범위를 잡고 돌려야해서 번거로웠다. 함수를 하나 더 빼서 범위를 잡고 그 범위를 기준으로 또 2차원 배열을 돌려 인덱스 계산을 했다. 
다음 단계인 녹는 조건 상, 하, 좌, 우 움직여서 범위를 벗어나는 것도 얼음이 없는 것이니 이 경우도 포함해 계산했다. 그리고 bfs로 얼음 덩어리 큰 거 뽑아내고, 마지막 얼음의 합산과 제일 큰 덩어리 크기 출력!
여기서 끝이 아닌게 덩어리가 없으면 둘 다 0으로 출력하는 게 마지막 난관이었다.
아직 골드는 갈길이 먼듯..;
