...
- 첫 접근은 행을 순회하면서 현재 요소와 다음요소의 크기를 비교 후 같을 때 개수 1차이라면 경사로 길이만큼 슬라이싱을 통해 다시 순회하는 방법을 했었는데, 문제는 작은 곳에서 큰 곳인지 큰 곳에서 작은 곳인지 구분하려면 너무 많은 분기가 필요했다
    
    풀이를 참고해 경사로의 사용여부를 선언해두고, 조건에 맞지 않으면 바로 탐색중단, 그리고 크기를 구분해 항상 작은 곳에서 큰 곳으로 가도록 한 부분이 인상깊었다
    
    원리 정리를 하면 생각보다  간단하게 구현할 수 있었는데 아쉬웠다
