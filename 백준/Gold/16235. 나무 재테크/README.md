...
문제를 읽고 나무 위치 배열과 현재 양분 배열, 기본 양분 배열이 필요하다고 생각했다. 양분 배열은 쉽게 구현했는데, 문제는 나무 위치 배열이었다. 2차원 배열로 끝이 아닌 여러 개의 나무가 한 곳에 있을 수 있고, 추가 되거나 삭제되는 부분 구현이 어려웠다. ArrayList로 2차원 배열을 만들어 주었고, 중간에 삭제, 추가 부분을 간과해 디버깅하는데 시간이 걸렸다. 탐색 도중 크기가 바뀌면 안된다는 부분을 다시 인지하고, 탐색도중 변하는 데이터를 따로 누적해 나중에 한번에 가산했다. 나무를 삭제하는 부분은 그냥 새로 ArrayList를 생성해 2차원 배열에 새로 값을 할당하는 방법을 사용했다. 삭제만 해야한다는 고정관념을 깨는 순간이었다.

4중 for문까지 나와서 시간복잡도가 불안했지만 총 입력 크기가 작은 편이라 그냥 진행했다.

기본 자료구조를 사용하며 새로운 시도를 해볼 수 있는 문제였다.
