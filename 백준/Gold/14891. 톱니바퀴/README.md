...
- 첫 구현 아이디어는 딕셔너리에 톱니바퀴의 번호와 현재 톱니바퀴 상태를 넣고, 주어지는 회전 방향에 따라 조건을 분기해서 결과를 출력하려 했었다. 하지만 회전기준 톱니가 회전한다해도 같은 극이라면 회전전파가 안 일어나는 문제가 있었다. 그럼 조건분기가 엄청 많아져서 새로운 아이디어를 찾았다.
    
    먼저 각 톱니위치의 회전방향을 어떻게 할 것인지 구한다음 한 번에 회전 시키는 방법이다.
    
    회전방향여부를 구하려면 for문을 이용해 극을 비교하고, 같은 극이면 더이상 회전전파가 안 일어나게 하는 것이다.
    
    구현 아이디어에서 시간이 좀 걸린 문제였다
