...
문제를 읽어보며 센서의 커버리지에 포함이 되는 부분을 뺀 나머지 부분의 수신영역 누적합을 구하는 문제였다.

문제 해석을 잘못해서 해결에 난항을 겪었고, 질문 게시판과 풀이를 참고해 이해했다. 먼저 센서 좌표 값을 정렬을 해주었고, 각 센서간 거리를 측정해 배열을 만들었다. 그리고 집중국은 센서사이에 있어야 하므로 각 센서들의 거리개수에서 집중국의 개수 - 1개를 빼준 나머지의 누적합을 구해 해결할 수 있었다.
