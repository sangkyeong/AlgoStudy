...
이중 우선순위 큐를 2개 사용해 명령어에 따라 가장 큰 값과 작은 값을 구하는 아이디어는 구현했는데 입력값이 많아 시간초과가 떴다. PriorityQueue는 remove가 O(n)이라 2개를 사용한다면 시간초과가 뜰 수 밖에 없었다. 추가구현을 참고해 remove를 위한 cnt map을 만들어준 후 개수로 최댓 값과 최솟 값을 판단하고 마지막 답을 도출할 때도 이를 이용해 출력하니 시간초과없이 잘 통과되었다.
