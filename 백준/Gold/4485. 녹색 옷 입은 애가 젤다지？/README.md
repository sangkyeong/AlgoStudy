...
첫 시작은 문제를 읽고 당연히 bfs로 접근하면 될 줄 알았다. 맨 처음엔 dfs로 풀었는데 역시나 시간초과가 떴고, bfs로 바꾸면서 최솟 값을 갱신하기 위해 큐를 어떻게 쓸지 고민하고 있었다. 계속 시간초과가 떠서 풀이를 참고했는데 다익스트라로 접근해야 문제를 풀 수 있었다ㅋㅋ

힙큐를 사용해 맨 처음 0, 0을 넣고, 제일 중요한 모든 칸을 최대 값으로 한 격자를 생성을 한 다음 최솟값을 갱신해나가는 방식으로 문제를 해결했다.

그래도 다익스트라사용 힌트를 얻고 바로 해결해볼 수 있었다. 이젠 당연하게 푼 것 방법들을 의심하며 효율성을 생각해야겠다..
