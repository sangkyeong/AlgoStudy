...
dp를 다시 복기하면서 문제를 풀었는데 최대 무게 기준으로 배열을 만들고,

중복으로 물건을 넣는 걸 방지하기위해 역순으로 순회한다는 것이 어려웠다.

점화식 부분도 이해하는데 오래 걸렸는데 결국은 현재 물건을 안 넣었을 때와 넣었을 때 (이미 앞 전 무게까지 생각) 가치를 판단해 갱신한다는 것을 잘 염두해 두고 문제를 많이 풀어봐야겠다..
