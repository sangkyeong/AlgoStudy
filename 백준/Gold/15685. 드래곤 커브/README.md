...
# 회고
좌표 위에 우, 상, 좌, 하 기준 초기 방향에서 그 다음 방향이 되는 규칙을 찾고, 다음 세대가 되면 이전 방향 상태에서 그 반대 방향이 되는 규칙을 찾았는데 격자 위에 표시하는 부분과 정사각형을 판단하는 부분, 코드로 변환해서 생각하는게 시간이 많이 소요되었다. 자료를 좀 더 찾아보니 100x100이라 완전탐색으로 단순하게 지나간 좌표들을 1로 처리후 네 꼭짓점이 되는지 판단해 문제를 해결할 수 있었다.

아이디어 구현에서 아직 미흡한 부분이 느껴졌던 문제였다.
