...
deque를 사용하면 배열로 구현한 것보다 훨씬 빠르다고 알고 있어서 사용했는데 시간초과가 떴었다..

혹시 deque를 사용하면 안되는 건가 해서 딕셔너리를 사용해 앞부분과 뒷부분 인덱스 변수를 두고 값을 넣고 빼고 조회하는 부분을 구현했다

하지만 논리오류가 있는지 계속 오류가 떴다

자료를 찾아보니 deque사용이 훨씬 빠르나 입력이 많아 시간초과가 뜨는 것이었다.

입력부분을 더 알아보니 한줄 읽어들이면 많은 데이터를 입력 받을 때 속도면에서 유리하다는 것을 알았다.
