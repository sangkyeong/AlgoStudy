...
완전탐색으로 그림이 있는 부분인 1을 먼저 찾으면 bfs를 이용해 그림이 연결되었는지 탐색을 한다.

탐색하며 그림의 넓이를 누적해 반환하고, 방문처리를 해준다. 그림연결 로직이 끝날 때마다 그림개수를 누적해주며 총 그림의 개수와 가장 넓은 그림을 출력해주었다.
